...
(defn process-steps
  ...
  (if (empty? steps)
    result
    (process-steps (rest steps) (concat result (process-step (first steps) (last result))))))

(process-steps ["R3" "U1" "L1"] [[0 0]])

(defn process-steps-str
  "process steps input as string"
  [steps-str]
  (process-steps (str/split steps-str #",") [[0 0]]))

(process-steps-str "R3,U1,L1")


(defn find-common-coordinates
  [input-one input-two]
  (let [coords-one (process-steps-str input-one)
        coords-two (process-steps-str input-two)]
    (clojure.set/intersection (set coords-one) (set coords-two))))

(find-common-coordinates "R8,U5,L5,D3" "U7,R6,D4,L4")
